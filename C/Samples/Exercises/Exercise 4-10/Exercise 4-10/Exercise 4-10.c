...
void mathcomm(char comm[])
{
	...
	if (strcmp(comm, "sin") == 0)
		...
		push(exp(pop()));
	else if (strcmp(comm, "pow") == 0 || strcmp(comm, "power") == 0 || strcmp(comm, "^") == 0) {
		int exp = pop();
		push(pow(pop(), exp));
	}
	else
...
int getop(char s[]) /* we pass s[] to getop to store input */
{
	...
	if (buf[bufp] == '\0') // if the limit of the string has been reached, overwrite.
		if (getline(buf, MAXSIZE) == 0)
			return EOF;
		else
			bufp = 0;
