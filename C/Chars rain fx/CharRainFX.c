...
{
	srand(time(NULL));
	int i, word_size, counter, rand_char, rand_int, rand_gap, value;

	while(1){
		if(!RANDOM(value, 2, NULL))
			change_color(1); //set value to 1 slows it down
		else
			change_color(NULL);

        word_size = RANDOM(rand_int, 150, 1);
		putchar(RANDOM(rand_char, 222, 33));
		counter++;
		if(counter >= word_size){ /* Word size */
